...
The purpose of this project review the current certificate program data to identify other new students that may be interested in the certificate program.


```{r load Libraries}
requiredPackages = c('RODBC','dplyr', 'stringr','tidyverse','caret','data.table','useful','lubridate','recommenderlab')
...
dbconnection <- odbcDriverConnect("Driver=ODBC Driver 11 for SQL Server;Server=OTIDB.hscnet.hsc.usf.edu; Database=OTIDatabaseNewOutreach_V3.2.0_BESQL;Uid=; Pwd=; trusted_connection=yes")
profCertsTrainers <- sqlQuery(dbconnection,paste("select TrainerID as t_id,ProfCertID,CertDate,Expires,IssueDate,ReCertifiedDate  from tblProfCertsTrainers;"))
trainers <- sqlQuery(dbconnection,paste("select t_id, t_lname, t_fname, CertificateProgram, hide   from tbltrainer;"))
certifications <- sqlQuery(dbconnection,paste("select * from ProfCerts;"))
certificationRequirements <- sqlQuery(dbconnection,paste("select * from ProfCertRequirements;"))
courseNames <- sqlQuery(dbconnection,paste("select id, CourseNumberText,credits from tblCourseNames;"))
odbcClose(dbconnection)


# str(ProfCertsTrainers)
# str(trainers)
# str(certifications)
# str(certificationRequirements)
# str(courseNames)

```

### Review structure and details of datasets  

```{r Review structure and details of dataset  }
unique_TrainerIDs <-unique(profCertsTrainers$t_id)
length(unique_TrainerIDs)

```
Join Tables

```{r Join tables}
# setkey(profCertsTrainers,t_id)
# setkey(trainers,t_id)

Certified_Trainers <- left_join(profCertsTrainers, trainers, by = "t_id")
glimpse(Certified_Trainers)
