...
```{r create smaller set to speed up processing, eval=FALSE, include=FALSE}
keep <- Student_data %>%
  dplyr::count(CourseID) %>%
  top_n(10) %>%
  pull(CourseID)

Student_data <- Student_data %>%filter(CourseID %in% keep)  
```

```{r Plot Course Rating Counts Top 10}
#Plot Course Rating Counts for the count of courses rated:  
Student_data %>% 
  dplyr::count(CourseID) %>%
  ggplot(aes(n)) + 
...
cat("Total Number of 1 course students:\t")
...
results <- evaluate(x = eval_sets, method = model_to_evaluate, n = seq(1, 40, 5))   
...
best_models <- append(UBCF_pea_model, IBCF_pea_model)
...
```{r Final IBCF Evaluation results, echo=FALSE}
