...
The purpose of this project (CYO) is to create recommending of classes for past users based on courses they have previously completed. I will be using a subset of modified data to protect the actual users infomation. I will use the recommenderlab for making my recommendation and use different models to identify the best model with the lowest error accuracy.
...
nrow(Student_data) #courses taking by students
ncol(Student_data)  #data columns
names(Student_data) #column Names
str(Student_data)  #structure of data
summary(Student_data) #stats per columns
class(Student_data) #dat.frame
unique_studentIds<-unique(Student_data$StudentID) # get unique Student IDS
students <- sample(unique(Student_data$StudentID),100) # get small sample of data to explore
...
### Preparing Data:   
```{r Clean data remove Student that have only taking 1 course, include=FALSE}
...
#The Distribution of Ratings shows we have different possible ratings from 70 to 100
  This data is based on the students who completed courses which reqires 70% which is apparent looking at the data.
...
*Note rating are fair even with the exception of 70 which appear could be given by instructor to pass a student.*
...
*Note with UBCF with 5 recommendation we get back 16700 recommendations of 1 item.*
*Note with UBCF with 10 recommendation we get back 33400 recommendations of 1 item.*
*Note with UBCF with 20 recommendation Error when creating evaluator schemes 'Some observations have size<given!'*
